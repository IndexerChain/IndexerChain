...
function App() {
  ...
  const DEFAULT_MAINNET_SIGNALING = "ws://localhost:8080"; // Dev: Local signaling server

  // Initialize to false so that restoreMining useEffect can trigger worker start
  ...
  // Removed syncMessage - light node mode doesn't need sync progress display
  ...
  // Provide a no-op setter for mining stats (stats are now tracked internally)
  ...
  // No-op setter for onboarding (onboarding is handled internally)
  ...
  useEffect(() => {
    const tryAutoBind = async () => {
      try {
        ...
        // IP hash removed - all nodes can mine without IP restrictions
        const ok = referralSystem.registerReferral(
          pendingInviteAddress as any,
          (nodeAddress as any),
          deviceId,
          undefined // IP hash no longer required
        );
        if (ok) {
            ...
            if (now - lastUnifiedSyncTime > 5000) {
              ...
              try {
                ...
                const syncResult = await handleRootTipUpdate(
                  ...
                  isMiner,
                  () => {
                    // Removed syncMessage - light node mode
                  }
                );
  ...
  const handleConnectP2P = async () => {
    ...
    try {
      ...
      console.log("[App] handleConnectP2P called");
      // Save bootstrap URL when connecting (state will be saved by useEffect)
      let nodeId = getOrCreateBrowserNodeId();

      // Dev: Append random suffix on localhost to allow multiple tabs to connect as distinct peers
      const isLocalhost = window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1";
      if (isLocalhost) {
        nodeId = `${nodeId}-${Math.random().toString(36).substring(2, 7)}`;
        logger.info(`[Dev] Using randomized NodeID for localhost P2P: ${nodeId}`);
      }
      ...
      if (params.signalServers && params.signalServers.length > 0) {
        ...
        let urlToUse = bootstrapUrl || DEFAULT_MAINNET_SIGNALING;
        // Force localhost if in dev environment and url is remote
        if (typeof window !== "undefined" && window.location.hostname === 'localhost' && (urlToUse.includes('workers.dev') || urlToUse.includes('indexerchain.com'))) {
          urlToUse = "ws://127.0.0.1:8080";
          setBootstrapUrl(urlToUse);
        }
        if (!urlToUse) {
      ...
      try {
        ...
        // Skip Shadow Node on localhost/dev environment to avoid 503 errors from simple signaling server
        const isLocalhost = bootstrapUrl?.includes("localhost") || bootstrapUrl?.includes("127.0.0.1");
  ...
  const handleStartMining = async () => {
            ...
            if (result.success) {
              ...
              if (newTip && typeof window !== "undefined") {
                ...
                if (chainContextRef.current) {
                  // Force update chain context ref to trigger React update
    ...
    try {
      ...
      logger.debug("[App] handleStartMining called");
                        ...
                        {syncStatus.networkHeight > 0 ? (
                          <>
                            <div className="status-item">
                              ...
                              <span className="value" style={{ fontSize: "1.2rem", fontWeight: "bold", color: "#28a745" }}>
                                {syncStatus.networkHeight}
                              </span>
                            </div>
                            {/* Light node mode - no sync progress needed, can mine immediately */}
                            <div style={{
                              marginTop: "0.5rem",
                              padding: "0.5rem",
                              background: "#d4edda",
                              ...
                              ✅ {locale === "zh" ? "轻节点模式：可立即挖矿和同步余额" : "Light node mode: Can mine and sync balance immediately"}
                            </div>
                          </>
                        ) : isP2PConnected ? (
                          <div style={{
                    ...
                    <div className="status-item">
                      ...
                      <span className="value">
                        {isConnecting
                          ...
                          : (isP2PConnected ? t("networkExpanded.connected") : t("networkExpanded.disconnected"))}
                      </span>
                    </div>
                    <div className="status-item">
                              ...
                              if (miningGuardResult.ok) {
                                switch (miningGuardResult.mode) {
                                  case "SAFE":
                                    ...
                                    break;
                                  case "GUARDED":
                                    // Generic guarded message (IP restrictions removed)
                                    const peerCount = miningGuardResult.details?.peerCount || 0;
                                    const minPeersRequired = chainContext?.params?.minPeersRequired ?? 3;
                                    const requiredPeers = miningGuardResult.details?.requiredPeers ?? minPeersRequired;
                                    statusMessage = `🟡 ${t("networkHealth.miningReady")}: ${t("networkHealth.guardedMode")} (${t("network.peers")}: ${peerCount} < ${requiredPeers})`;
                                    break;
                                  case "LOCAL_ONLY":
                  ...
                  {!isP2PConnected ? (
                    <div style={{ marginTop: "1rem", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                      ...
                      <div style={{ display: "flex", gap: "0.5rem" }}>
                        ...
                        />
                        <button className="btn btn-primary" onClick={handleConnectP2P} disabled={isConnecting}>
                          {isConnecting ? (locale === "zh" ? "连接中..." : "Connecting...") : t("network.connect")}
                        </button>
                      </div>
                      {isMainnetMode && (
                    ...
                    <div style={{ marginTop: "1rem", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                      ...
                      <button
                        className="btn btn-primary"
                        onClick={async () => {
                          await handleDisconnectP2P();
                          setTimeout(handleConnectP2P, 500);
                        }}
                        disabled={isConnecting}
                      >
                        {isConnecting ? (locale === "zh" ? "重连中..." : "Reconnecting...") : (locale === "zh" ? "强制重连" : "Force Reconnect")}
                      </button>
                    </div>
                  )}
                  {p2pNodeRef.current && (() => {
                    ...
                    <div className="status-card" style={{ background: "#d4edda", border: "2px solid #28a745" }}>
                      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                        ...
                        <div>
                          ...
                          <p style={{ margin: "0.5rem 0 0 0", color: "#155724", fontSize: "0.9rem" }}>
                            {locale === "zh"
                              ? "在线时长系数（0.5x-1.2x）和连续登录奖励，鼓励长期稳定在线。所有节点都可以挖矿，无IP限制。"
                              : "Session duration (0.5x-1.2x) and active booster multipliers, encouraging long-term stability. All nodes can mine without IP restrictions."}
                          </p>
                        </div>
                        <div>
